...
Реализуем простейшую стратегию. Она будет состоять в следующем: будем поддерживать наши заявки на лучшей цене в обоих направлениях. Сначала научимся ставить заявку. Для этого предназначена функция [add_order](../../api/ParticipantStrategy#add_order):
