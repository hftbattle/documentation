...
- Помимо симулятора торгов мы предоставляем вам [Viewer]({{ book["viewer.url"] }}) для визуализации торгов. В нем можно будет посмотреть что происходило на выбранном инструменте в любой момент любого дня из набора A. Вьювер доступен по адресу [{{ book["viewer.name"] }}]({{ book["viewer.url"] }}).
