...
- Помимо симулятора торгов мы предоставляем вам инструмент для визуализации торгов - [вьювер]({{ book["viewer.url"] }}). В нем можно будет посмотреть что происходило на выбранном инструменте в любой момент любого дня из набора A. Вьювер доступен по адресу [{{ book["viewer.url"] }}]({{ book["viewer.url"] }}).
