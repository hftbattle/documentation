...
Пустая стратегия выглядит следующим образом. Здесь тоже есть три метода, которые вы можете реализовать, а также метод *init*, который позволяет получать инорфмацию из файла конфигурации или же из веб-интерфейса.
