...
В предыдущей стратегии мы ставим заявку на уровень, который отстоит от средней цены middle_price на фиксированную разницу `offset_`.
Однако, такое поведение не является разумным, если на одном из направлений лучшая цена является неадекватной.
middle_price очень чувствителен по отношению к лучшим ценам — если появится даже одна заявка с ценой лучше, чем была, то изменится middle_price и изменятся уровни, на которые мы ставим заявку.

Отметим, что стакан является разреженным — между котировками могут быть большие промежутки (т.е. большое количество ценовых уровней без заявок), а также на многих котировках стоит небольшой объём.
...
2. Помимо ограничения на объём перед нашей заявкой, стоит проследить за тем, чтобы наши заявки не стояли близко друг к другу (на расстоянии меньше двух комиссий).
  Если обе заявки исполнятся, то мы окажемся в минусе, просто заплатив комиссию.
...
1. Мы всё ещё торгуем с помощью лишь одной заявки.
2. Наша стратегия только котирует заявки — мы сами не совершаем активных действий.
3. Возможно, стоит подумать о том, какая цена является справедливой в каждый момент времени.
  Как мы поняли, middle_price является очень плохим показателем.
4. Динамически менять ценовой уровень наших заявок в зависимости от текущей позиции — это позволит быстрее закрывать позицию в тех случаях, когда она большая по модулю.
